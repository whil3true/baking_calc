...
const PortionCalc = (() => {
  ...
  function portionDimensions(input, errors, values) {
    ...
    if (!Number.isFinite(width) || width <= 0 || width > 30) errors.portionWidth = 'Ширина порции: введите число больше 0 и до 30 см.';
    if (!Number.isFinite(length) || length <= 0 || length > 30) errors.portionLength = 'Длина порции: введите число больше 0 и до 30 см.';
  ...
  function calculate(input) {
    ...
    const step = parseNumber(input.roundStep);
    if (!Number.isFinite(step) || step <= 0 || step > 10) errors.roundStep = 'Шаг округления: введите значение больше 0 и до 10 см.';
    values.roundStep = step;
    ...
    if (input.shape === 'circle') {
      ...
      const diameter = roundUp(rawDiameter, step);
      const actualArea = Math.PI * Math.pow(diameter / 2, 2);
      const capacity = Math.floor(actualArea / portionArea + 1e-9);
      result = { shape: 'circle', rawDiameter, diameter, actualArea, capacity };
    } else {
      const rawWidth = Math.sqrt(requiredArea / ratio);
      const rawLength = rawWidth * ratio;
      let width = roundUp(rawWidth, step);
      let length = roundUp(rawLength, step);
      let actualArea = width * length;
      // Independent rounding can theoretically undershoot due to unusual ratios/steps.
      while (actualArea + 1e-9 < requiredArea) {
        length = roundUp(length + step, step);
        actualArea = width * length;
      }
      const capacity = Math.floor(actualArea / portionArea + 1e-9);
      result = { shape: 'rect', rawWidth, rawLength, width, length, actualArea, capacity, ratio };
    }
  ...
  return Object.freeze({ defaults, presets, parseNumber, roundUp, restoreSettings, calculate });
})();
